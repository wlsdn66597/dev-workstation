...
├── docs/<br>
│   └── screenshots/<br>
│       ├── terminal-basic.jpeg <br>
│       ├── permission-before-after.jpeg<br>
│       ├── docker-version.jpeg<br>
│       ├── docker-info.jpeg<br>
│       ├── docker-ops.jpeg<br>
│       ├── hello-world.jpeg<br>
│       ├── ubuntu-container.jpeg<br>
│       ├── docker-build.jpeg<br>
│       ├── docker-run.jpeg<br>
│       ├── port-mapping.jpeg<br>
│       ├── bind-run.jpeg<br>
│       ├── bind-before.jpeg <br>
│       ├── bind-after.jpeg <br>
│       ├── volume-create.jpeg <br>
│       ├── volume-before-delete.jpeg<br>
│       ├── volume-after-delete.jpeg <br>
│       ├── git-config.jpeg <br>
│       ├── github-repo.jpeg<br>
│       └── github-vscode-login.jpeg <br>
├── Dockerfile<br>
└── README.md
...
cd practice<br>
touch empty.txt<br>
echo "hello" > note.txt<br>
cat note.txt<br>
cp note.txt copy.txt<br>
mv copy.txt renamed.txt <br>
mkdir subdir<br>
mv renamed.txt subdir/<br>
ls -la<br>
rm empty.txt<br>
cd ..<br>
rm -r practice<br>
...
7-4. 증거
- Docker 버전 확인 화면: ![docker-version](docs/screenshots/docker-version.jpeg)
- Docker info 확인 화면: ![docker-info](docs/screenshots/docker-info.jpeg)
...
docker ps<br>
docker ps -a<br>
docker logs my-nginx-container<br>
...
8-4. 증거
운영 명령 실행 화면: ![docker-ops](docs/screenshots/docker-ops.jpeg)
...
9-5. 증거
- hello-world 실행 화면: ![hello-world](docs/screenshots/hello-world.jpeg)
- Ubuntu 컨테이너 실행 화면: ![ubuntu-container](docs/screenshots/ubuntu-container.jpeg)
...
10-7. 증거
- Docker build 화면: ![docker-build](docs/screenshots/docker-build.jpeg)
- 컨테이너 실행 화면: ![docker-run](docs/screenshots/docker-run.jpeg)
...
- 브라우저 접속 캡처(주소창 포함): ![port-mapping](docs/screenshots/port-mapping.jpeg)
...
- 실행 명령 화면: ![bind-run](docs/screenshots/bind-run.jpeg)
- 수정 전 화면: ![bind-before](docs/screenshots/bind-before.jpeg)
- 수정 후 화면: ![bind-after](docs/screenshots/bind-after.jpeg)
...
docker run-it--name volume-test-v mydata:/data ubuntubash <br>

컨테이너 내부에서:
echo"persistent data" > /data/test.txt <br>
cat /data/test.txt<br>
exit <br>

컨테이너 삭제 후 다시 확인:
dockerrm-f volume-test <br>
docker run-it--name volume-test-2-v mydata:/data ubuntubash <br>
cat /data/test.txt <br>
exit<br>
...
git config--global user.name"jinu" <br>
git config--global user.email"1@2.com"<br>
...
git add .  <br>
git commit-m"Initial mission setup" <br>
git branch-M main <br>
git remote add origin https://github.com/wlsdn66597/dev-workstation.git <br>
...
14-4. 증거
- git config 결과: ![git-config](docs/screenshots/git-config.jpeg)
- GitHub 저장소 화면: ![github-repo](docs/screenshots/github-repo.jpeg)
- VSCode GitHub 연동 화면: ![gitgub-vscode-login](docs/screenshots/github-vscode-login.jpeg)

### 15. 트러블슈팅

문제 1. docker info가 실행되지 않음

- 문제: `docker info` 실행 시 Docker 엔진에 연결되지 않았다.
- 원인 가설: OrbStack이 실행되지 않은 상태였을 가능성이 있다.
- 확인: OrbStack 애플리케이션이 실행 중인지 확인하였다.
- 해결: OrbStack을 실행한 뒤 다시 `docker info`를 실행하니 정상 동작하였다.

### 문제 2. 브라우저에서 localhost:8080 접속이 되지 않음

- 문제: 컨테이너는 실행되었지만 웹 페이지 접속이 되지 않았다.
- 원인 가설: 포트 매핑이 잘못되었거나 컨테이너가 정상 실행되지 않았을 가능성이 있다.
- 확인: `docker ps`를 통해 `0.0.0.0:8080->80/tcp` 매핑 여부와 컨테이너 상태를 점검하였다.
- 해결: `p 8080:80` 옵션으로 컨테이너를 다시 실행하여 접속에 성공하였다.